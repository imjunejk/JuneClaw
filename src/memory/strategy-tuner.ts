...
/**
 * Run the strategy tuning cycle. Called after Dream consolidation.
 */
    ...
    if (
      ...
      if (oldStats && newStats) {
        ...
        if (improved) {
          ...
          await emit("tuner:keep", { taskType: state.targetTaskType, delta: newStats.avg - oldStats.avg });
          ...
          console.log(msg);
          await appendSystemLog(msg);
          await emit("tuner:keep", { taskType: state.targetTaskType, delta: 0 });
          state.previousHash = state.currentHash;
        } else {
          ...
          if (reverted) {
            ...
            state.totalReverts++;
            justReverted = true;
            await emit("tuner:discard", { taskType: state.targetTaskType });
...
Output one block per file. Only include files you changed.`;
    ...
    try {
      ...
      state.totalTunes++;
      await saveTunerState(state);
      await emit("tuner:tuned", { taskType, filesUpdated });
