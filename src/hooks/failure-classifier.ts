 ...
 *   - timeout:     Claude CLI exceeded time limit
 *   - context:     Context window exhaustion
 *   - model:       Circuit breaker / model unavailability
 *   - parse:       Non-JSON or unexpected output from Claude
 *   - spawn:       Process spawn failures
...
const CATEGORY_PATTERNS: { category: FailureCategory; pattern: RegExp }[] = [
...
function classifySymptom(symptom: string): FailureCategory {
...
export function clusterFailures(failures: ClassifiedFailure[]): FailureCluster[] {
  const groups = new Map<FailureCategory, ClassifiedFailure[]>();
 ...
 * Called from the Dream cycle or cron scheduler.
 */
 ...
 * Returns true if re-classification is needed.
 */
export async function needsReclassification(): Promise<boolean> {
  try {
    ...
    // If either file doesn't exist, check if incidents exist at all
    try {
